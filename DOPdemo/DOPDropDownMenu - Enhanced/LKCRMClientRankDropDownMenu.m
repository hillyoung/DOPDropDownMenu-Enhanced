...
#import "LKCRMClientRankDropDownMenu.h"

@interface LKCRMClientRankDropDownMenu () {
    __weak id<DOPDropDownMenuDataSource> _dataSource;
}

@end

@implementation LKCRMClientRankDropDownMenu
@dynamic dataSource, delegate, indicatorColor, textColor, separatorColor;
...
- (id<DOPDropDownMenuDataSource>)dataSource {
    return _dataSource;
}
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        bottomShadow.backgroundColor = [UIColor groupTableViewBackgroundColor];
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    if ([self.delegate respondsToSelector:@selector(menu:viewForFooterInColumn:)]) {
        footer = [self.delegate menu:self viewForFooterInColumn:tapIndex];
    }
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        if (self.tableView.tableFooterView) {
            tableViewHeight += CGRectGetHeight(self.tableView.tableFooterView.frame);
        }
...
- (void)confiMenuWithSelectRow:(NSInteger)row {
    ...
    if(_currentSelectedMenudIndex == 0)
    {
        ...
        title.string = [self.dataSource menu:self titleForRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex + 1 row:0]];
    }
